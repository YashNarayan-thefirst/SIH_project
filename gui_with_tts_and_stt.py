...
def submit(output='',file_path=''):
    dest()
    inp = (entry_1.get(1.0, "end-1c"))#user input
    output_display= resp(inp) if output=='' else output#output recieved from chatbot
...
def filediag():
    ...
    submit(stt(file_path),file_path)
